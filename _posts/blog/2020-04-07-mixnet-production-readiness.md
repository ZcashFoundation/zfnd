...
As described in the prior section, a critical consideration to keep in mind as mixnets mature is the size of its anonymity set. Even though mixnets inject latency (and possibly cover traffic) as packets move through the network, the size of the mixnet’s user base still directly impacts the resulting privacy of users of that mixnet. So even if a large-scale mixnet did exist, considering the amount of traffic and number of users still is important to understanding the size of the anonymity set that protects each user.

Consequently (and perhaps unintuitively), in the case of anonymity networks, fewer options is truly better. This is because of the "strength in numbers" property, as the privacy a user enjoys when using an anonymity network is directly related to the number of other users also using that network. In other words- an adversary observing an anonymity network must distinguish between my traffic and traffic generated by all other users of that network; but the success of this adversary is directly increased if the total number of other users is five versus five million.
